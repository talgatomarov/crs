...
with pm.Model() as model:
    ...
    sigma = pm.HalfNormal("sigma", sigma=50, shape=k)
...
To ensure flexibility while keeping the model grounded, I chose relatively uninformative but reasonable hyperpriors. 
- The Normal prior on `mu` centers the component means around 450 with a standard deviation of 50, which places about 95% of the prior mass between 350 and 550. This range covers the majority of CRS scores where candidates tend to cluster. 
- The HalfNormal prior on `sigma` with a standard deviation of 50 allows for plausible variability in spread while avoiding unrealistically wide components. 
- Finally, the symmetric Dirichlet prior on `w` reflects no strong preference for how weights are distributed among components, encouraging the model to infer structure from the data rather than the prior.
...
The trace plots show that all parameters (means mu, spreads sigma, and weights w) stabilize quickly and fluctuate steadily without drift. This indicates good convergence and consistent sampling. The chains for each mixture component remain well separated, showing that label switching is not occurring. Label switching refers to components in a mixture model becoming indistinguishable during sampling, which can distort interpretation. Overall, the traces are well behaved, with no signs of divergence or poor mixing, so the posterior samples appear reliable.

```{python}
az.summary(idata, var_names=["mu", "sigma", "w"]).round(2)
```

All R-hat values are close to 1, with the largest at 1.03. This suggests good convergence across chains. Values below 1.1 are generally considered acceptable, and those near 1.0 indicate that the chains have mixed well and are sampling from the same posterior distribution.
